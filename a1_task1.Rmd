...
```{r}
p4 <- ggplot(data = palmetto_sub, aes(x = green_lvs, 
                                      fill = species)) +
  geom_histogram() +
  facet_wrap(~species) + #separating by species
  theme_minimal() +
...
results_table #calling out
```

# Conclusion
***
Model 1 included the variables plant height, canopy length, canopy width and green leaves was proven to statistically predict species better than model 2. This was supported through AIC, BIC, and 10-fold cross validation results. As shown in Table 7, the percent correctly classified as *Serenoa repens* `r results_table[1,3]` 

`r round((blr1_accuracy[1,4] + blr1_accuracy[2,4])/2, 2)`%
 `r palmetto_table[1,4]` 
